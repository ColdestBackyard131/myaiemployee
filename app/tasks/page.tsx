...
export default function TasksPage() {
  const [tasks, setTasks] = useState<Task[]>([
    { id: "1", text: "Review AI Employee dashboard design", done: false, priority: "high", date: new Date().toISOString().split("T")[0] },
    { id: "2", text: "Write YouTube script about productivity", done: false, priority: "medium", date: new Date().toISOString().split("T")[0] },
  ]);
  ...
  const addTask = () => {
    ...
    setTasks([...tasks, { id: Date.now().toString(), text: input, done: false, priority, date }]);
    setInput("");
  };
  ...
  const handleAiTask = async () => {
    ...
    setTasks([...tasks, { id: Date.now().toString(), text: data.reply, done: false, priority: "medium", date }]);
    setAiInput("");
  };
          ...
          <div className="flex flex-col gap-3">
            ...
            {pending.map((task) => (
              <div key={task.id} className="glass rounded-2xl p-4 flex items-center gap-4 hover:scale-[1.01] transition-all">
                ...
                <button onClick={() => deleteTask(task.id)} style={{ color: "#ef4444" }}>
                  <Trash2 size={16} />
                </button>
              </div>
            ))}
